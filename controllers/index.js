...
module.exports.indexPage = function(req, res) {
    ...
    var queriedWord = req.query.define,
        queriedLanguage = req.query.lang || 'en';

    if (!queriedWord) {
        res.sendFile(path.join(__dirname, '../views//index.html'));
    } else {
        queriedWord = queriedWord.toLowerCase();
        queriedLanguage = queriedLanguage.toLowerCase();

        console.log(queriedWord);


        Word.find({ word: queriedWord, lang: queriedLanguage }, function(err, words) {
            if (err) {
                console.log(err);
            } else {
                if (words.length == 0) {
                    //i.e word isn't in database... so scrape and add to database
                    ...
                    if (queriedLanguage !== 'en') {
                        ...
                        request({
                            ...
                            Word.create({
                                word: queriedWord,
                                lang: queriedLanguage,
                                meaning: JSON.stringify(dictionary, null, 4)
                            }, function(err, cat) {
                                if (err) {
                                    console.log(err);
                                } else {
                    ...
                    //word is in database. no need to scrape. Just simply pick from database
                    res.send(words[0].meaning);
                    console.log("Word fetched from DB");
                }
